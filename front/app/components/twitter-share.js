...
export default Ember.Component.extend({
  ...
  webIntentUrl: Ember.computed('useWebIntent', 'url', 'text', 'via', 'related', 'hashtags', 'urlFix', function(){
  ...
  createTwitterShareButton: Ember.on('twitterLoaded', function() {
    if (this.get('useWebIntent')) { return; }
    var urlFixed = '';
    console.log(this.get('urlFix'));
    if (this.get('urlFix')) {
    	urlFixed = window.location.href + '/' + this.get('urlFix') + '/show';
    }
